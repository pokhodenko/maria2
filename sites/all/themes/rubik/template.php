...
function rubik_preprocess_page(&$vars) {
  ...
  if (drupal_is_front_page()) { // If the page is in the checkout area
    $vars['template_files'][] = 'page_front';
  }

  $path = drupal_get_path_alias($_GET['q']);
  if (drupal_match_path($path, 'gallery/*')) {
    $vars['template_files'][] = 'page_gallery';
  }
  /*elseif(in_array($_GET['q'], $print_paths)){  // If the page is in the print path array
    $vars['template_files'][] = 'page-print_legal';

  } else { // If the page is in the my-account area or main site

    $vars['template_files'] = array('page-mainsite');
